...
set local role postgres;
update public.election_cycles set status='failed',completed_at=now() where id=(select active_election_cycle_id from public.communities where id='56100000-0000-4000-8000-000000000003');
set local role authenticated;
...
select throws_ok($$select public.commit_democratic_transfer('56100000-0000-4000-8000-000000000001')$$,
 '55000','At least three candidates are required','zero candidates cannot commit');
...
-- Complete the already-frozen election through internal engine data, then install internally.
set local role postgres;
update public.election_cycles set status='completed',completed_at=now() where id=(select active_election_cycle_id from public.communities where id='56100000-0000-4000-8000-000000000001');
insert into public.election_winners(cycle_id,candidate_id,elected_in_round,approval_count)
select c.active_election_cycle_id,u,1,3 from public.communities c,
 unnest(array['56000000-0000-4000-8000-000000000003'::uuid,'56000000-0000-4000-8000-000000000004','56000000-0000-4000-8000-000000000005'])u
where c.id='56100000-0000-4000-8000-000000000001';
select lives_ok($$select public.install_elected_council('56100000-0000-4000-8000-000000000001',(select active_election_cycle_id from public.communities where id='56100000-0000-4000-8000-000000000001'))$$,'completed final winners install a council');
...
select throws_ok($$select public.close_election_round((select id from public.election_rounds limit 1))$$,
