...
update cycles set round_id=(select id from public.election_rounds where cycle_id=cycles.id and round_number=2);
set local role authenticated;
select set_config('request.jwt.claim.sub','56000000-0000-4000-8000-000000000001',true);
select lives_ok(format('select public.submit_election_ballot(%L,array[%L::uuid])',(select round_id from cycles),'56000000-0000-4000-8000-000000000003'),'same elector may cast a fresh runoff ballot');
select set_config('request.jwt.claim.sub','56000000-0000-4000-8000-000000000002',true);
select lives_ok(format('select public.submit_election_ballot(%L,array[%L::uuid])',(select round_id from cycles),'56000000-0000-4000-8000-000000000003'),'second runoff ballot');
select set_config('request.jwt.claim.sub','56000000-0000-4000-8000-000000000003',true);
select lives_ok(format('select public.submit_election_ballot(%L,array[%L::uuid])',(select round_id from cycles),'56000000-0000-4000-8000-000000000004'),'third runoff ballot reaches quorum');
set local role postgres;
select public.close_election_round((select round_id from cycles));
select is(public.finalize_election_round((select round_id from cycles))::text,'completed','runoff resolves the cycle');
