...
select ok((select bool_and(can_decide) from public.list_booking_requests()), 'active owner receives decision capability metadata');
...
select is((select count(*) from public.list_booking_requests()), 3::bigint, 'active same-community admin can see decision requests');
select ok((select bool_and(can_decide) from public.list_booking_requests()), 'active same-community admin receives decision capability metadata');
