...
create function public.can_upload_condition_photo(
  object_name text,
...
as $$
  select exists (
    ...
    where r.photo_path = object_name
      ...
      and r.mime_type = object_mime_type
      and auth.uid() in (b.borrower_id, i.owner_id)
      and (
        (r.phase = 'before' and b.status = 'accepted')
        or (r.phase = 'after' and b.status = 'checked_out')
      )
  );
$$;

comment on function public.can_upload_condition_photo(text, text) is
  'Narrow Storage authorization helper that keeps condition report author and MIME metadata private.';

revoke all on function public.can_upload_condition_photo(text, text) from public;
grant execute on function public.can_upload_condition_photo(text, text) to authenticated;

create policy "participants can upload reserved condition photos" on storage.objects
for insert to authenticated with check (
  bucket_id = 'condition-photos'
  and public.can_upload_condition_photo(name, metadata->>'mimetype')
);
