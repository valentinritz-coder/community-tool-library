...
with scoped_items as (
  ...
  select owner_id,count(*) n from eligible_window_items group by owner_id
), borrower_counts as (
  select borrower_id,count(*) n from window_bookings group by borrower_id
), report_count as (
...
select
 ...
 (select n from report_count) incidents_as_reports,
 ...
 round((select n from report_count)/nullif((select n from request_count),0),4) reports_per_request_context;
